<template>
    <div class="container">
        <div class="question-content" v-bind="currentQuestion" v-if="!isFetching">
            ...
            <img class="question-image" :src="`api/questions/${currentQuestion.id}/image`" v-if="currentQuestion.media.length > 0">
            ...
            <div class="answers-container disable-select">
                <a
                    ...
                    :class="[
                        'answer',
                        answer.id === selectedAnswers ?'selected-answer' : '',
                    ]"
...
<script>export default {
    created() {
        axios.get('api/get-questionnaire').then((response) => {
            ...
            if (this.SKIP_ANSWERS) {
                this.questions.forEach((question, index) => {
                    this.userResults[index] = {
                        'question_id':question.id,
                        'answers':question.answers[0].id
                    };
                });
                this.selectedAnswers = this.questions[0].answers[0].id;
            }
    ...
    computed: {
        canSaveAnswer() {
            return !!this.selectedAnswers && !this.answerIsSaved;
        },

        answerIsSaved() {
            return !!this.userResults[this.currentIndex] ?
                this.userResults[this.currentIndex].answers == this.selectedAnswers :
                false;
        },
        ...
        hasAnsweredAll() {
            return Object.keys(this.userResults).length == this.questions.length;
        }
    },
    ...
    methods: {
        answerClicked(answerId) {
            this.selectedAnswers.push({
                ...
                [].push(answerId);
            // if (this.selectedAnswers != answerId) {
            //     this.selectedAnswers = answerId;
            // }
        },

        saveAnswer() {
            //Save selected answer in userResults
            this.userResults[this.currentIndex] = {
        ...
        goToQuestion(index) {
            ...
            this.selectedAnswers = !!this.userResults[this.currentIndex] ? this.userResults[this.currentIndex].answers : null;
        },

        submitQuestionnaire() {
            axios.post('api/questionnaire-runs', this.userResults).then(response => {
                this.$router.push({name:'userResults', params: {questionnaire_id: response.data }});
            });
        },
    },
}
